...
image as the shim. A session is now a process tree in the ExApp
container, not a sibling container — an ExApp cannot start one of those.
`design.md` carries the decision, its cost, and the two routes back to
container isolation.

Proof it works, on NC 32.0.14 + AppAPI 5.x + HaRP v0.4.0: a 60-byte DOS
executable generated by `scripts/make-dos-fixture.py`, uploaded to
Files, started through the AppAPI proxy — `dosbox-x` ran it and wrote
`ASH NAZG OK`. The level-3 verifier asserts that path end to end,
including close-then-run-again.

- **Runtime image** is now Ubuntu 24.04 carrying uvicorn, `dosbox-x`,
  KasmVNC and `frpc` (795 MB, up from 262 MB). The venv is built on the
  same base it runs on so its interpreter path resolves.
- **`InImageSpawner`** starts one process tree per session: a private
  0700 directory holding only the binary (downloaded, not mounted — no
  davfs2, no mount privileges), its own X display and websocket port, a
  cap of 8 concurrent sessions, `nice +10`, a 4-hour maximum duration,
  and SIGTERM → 30 s → SIGKILL on close. `DockerSubprocessSpawner` is
  deleted: it could never work from inside an ExApp.
- **`DELETE /sessions/{id}`** ends a session and releases the (user,
  file) claim. Without it a user who ran a file once could never run it
  again — nothing released that claim before.
- **A session page** (`GET /sessions/{id}` + `SessionStatus.vue`) is
  where the Files action now navigates. It shows the session and offers
  to close it; the picture arrives with `streaming-proxy`.
- **The Files action** calls `POST /run` and surfaces the host's own
  error text on failure.
- **`/run` answers 404** for a file it cannot read, instead of 500 with
  a WebDAV transport error.
- **The `engine-runtime` self-check** replaces `deploy-daemon-spawn`:
  it verifies the emulator and VNC server are in this image and a
  session slot is free. All four self-checks now pass live.

The security model and the installation guide were rewritten where they
promised isolation this design does not provide: no cgroup limits, no
read-only root, and a session running under the shim's uid. What remains
is the emulator — the binary is a DOS program under DOSBox-X, never
native code — and that is now stated as the boundary it is.
